...
#   - `tf_quaternion.transformations` replaced with `tf_transformations`
#     (package: ros-<distro>-tf-transformations)
#   - Removed duplicate `return True` at end of init_interpolator
...
class CSInterpolator(PathGenerator):
    """Interpolator that will generate cubic Bezier curve segments for a set
    of waypoints. The full algorithm can be seen in
    `Biagiotti and Melchiorri, 2008`.
    ...
    def get_samples(self, max_time, step=0.001):
        ...
        * `step` (*type:* `float`, *default:* `0.001`): Sampling step
    ...
    def generate_pos(self, s):
        ...
        interpolated on the position related to `s`.

        > *Input arguments*

        * `s` (*type:* `float`): Curve's parametric input in [0, 1]
    ...
    def generate_pnt(self, s, t, *args):
        """Compute a point that belongs to the path on the interpolated space.
        ...
        * `t` (*type:* `float`): Trajectory point's timestamp
        ...
        `uuv_trajectory_generator.TrajectoryPoint`
        """
        pnt = TrajectoryPoint()
        pnt.t = t
    ...
    def generate_quat(self, s):
        """Compute the quaternion of the path reference for an interpolated
        point related to `s`.
        ...
        * `s` (*type:* `float`): Curve's parametric input in [0, 1]
        ...
        Rotation quaternion as `numpy.array` as `(x, y, z, w)`
        """
        s = max(0, s)
        s = min(s, 1)
