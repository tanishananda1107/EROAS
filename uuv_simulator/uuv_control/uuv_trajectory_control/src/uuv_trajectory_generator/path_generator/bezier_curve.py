...
class BezierCurve(object):
    ...
    * `pnts` (*type:* `list`): List of 3D points (e.g. `[[0,0,0],[0,1,2]]`)
    * `order` (*type:* `int`): Order of the Bezier curve — 3, 4 or 5
    * `tangents` (*type:* `list`, *default:* `None`): Optional tangent vectors
    * `normals` (*type:* `list`, *default:* `None`): Optional normal vectors
    ...
    def __init__(self, pnts, order, tangents=None, normals=None):
        ...
        assert type(pnts) == list and len(pnts) >= 2, \
        ...
        if tangents is not None:
            assert type(tangents) == list and len(tangents) == 2, \
        ...
        if self._order == 3:
            ...
            b = 12 * np.dot(
                self._control_pnts[3] - self._control_pnts[0],
                tangents[0] + tangents[1])
            c = -36 * np.linalg.norm(
                self._control_pnts[3] - self._control_pnts[0])**2
            ...
            b = 3 * np.dot(
                self._control_pnts[4] - self._control_pnts[0],
                tangents[0] + tangents[1])
            c = -9 * np.linalg.norm(
                self._control_pnts[4] - self._control_pnts[0])**2
            alpha = np.roots([a, b, c]).max()

            self._control_pnts[1] = (
                self._control_pnts[0] + 0.25 * alpha * tangents[0])
            self._control_pnts[3] = (
                self._control_pnts[4] - 0.25 * alpha * tangents[1])
            ...
            if len(self._pnts) == 3:
                ...
                b = 420 * np.dot(
                    ...
                    tangents[0] + tangents[1])
                c = -900 * np.linalg.norm(
                    self._control_pnts[5] - self._control_pnts[0])**2
                alpha = np.roots([a, b, c]).max()
                ...
                assert isinstance(tangents, list) and len(tangents) == 2, \
                    'Tangent vectors must be provided'
                ...
                b = -28 * beta_hat * np.dot(
                    (tangents[0] + tangents[1]), normals[1] - normals[0])
                c = (196 * np.linalg.norm(tangents[0] + tangents[1])**2
                     + 120 * beta_hat * np.dot(
                         self._pnts[1] - self._pnts[0], normals[1] - normals[0])
                     - 1024)
                d = -1680 * np.dot(
                ...
                self._control_pnts[2] = (
                    2.0 * self._control_pnts[1] - self._control_pnts[0]
                    + beta_hat * alpha_k**2 / 20.0 * normals[0])
                self._control_pnts[4] = (
                    self._control_pnts[5] - alpha_k / 5.0 * tangents[1])
                self._control_pnts[3] = (
                    2.0 * self._control_pnts[4] - self._control_pnts[5]
    ...
    def generate_cubic_curve(pnts):
        ...
        lengths = [BezierCurve.distance(pnts[i + 1], pnts[i])
    ...
    def interpolate(self, u):
        """Interpolate the Bezier curve at parametric variable `u` in [0, 1]."""
        u = max(u, 0)
        u = min(u, 1)
        b = np.zeros(3)
